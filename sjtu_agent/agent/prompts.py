...
- 用户说「树洞」「YKST」「treehole」→ 优先使用原生 YKST 工具；搜索帖子用 ykst_search_threads，读主帖用 ykst_get_thread，读回帖用 ykst_get_thread_posts。若工具提示未登录，调用 setup_ykst 引导登录。
...
- **树洞 / YKST 原生操作**：
  ...
  - 登录/配置：用户说「配置树洞」「登录树洞」「配置 YKST」→ setup_ykst；用户贴回调 URL 或 OAuth code → ykst_login_with_callback。
  - 查登录态/身份：ykst_auth_status、ykst_get_profile、ykst_list_identities、ykst_get_identity。
  - 搜索/阅读：ykst_search_threads(keyword=...)，读主题 ykst_get_thread(thread_id=...)，读回帖 ykst_get_thread_posts(thread_id=...)，读单条回帖 ykst_get_post(post_id=...)。
  - 写操作必须二次确认：切换身份、回帖、点赞/点踩/取消、收藏/取消收藏都先展示草稿或目标，再等用户明确确认后带 confirm=true 调用。
  - **严禁编造树洞内容**：用户询问某个树洞帖或回帖内容时，必须先调用对应 ykst 工具读取真实内容。
...
_TOOL_LABELS = {
    ...
    "setup_ykst":              "正在生成树洞登录链接",

    "ykst_login_with_callback": "正在保存树洞登录态",

    "ykst_auth_status":         "正在检查树洞登录态",

    "ykst_get_profile":         "正在读取树洞账号信息",

    "ykst_list_identities":     "正在列出树洞身份",

    "ykst_get_identity":        "正在读取树洞身份",

    "ykst_set_active_identity": "正在切换树洞身份",

    "ykst_search_threads":      "正在搜索树洞帖子",

    "ykst_get_thread":          "正在读取树洞帖子",

    "ykst_get_post":            "正在读取树洞回帖",

    "ykst_get_thread_posts":    "正在读取树洞回帖列表",

    "ykst_reply_thread":        "正在发布树洞回帖",

    "ykst_rate_thread":         "正在更新树洞帖子态度",
