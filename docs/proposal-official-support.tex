...
\vspace{0.3em}
\textbf{学生维护、开源、零经费。项目主要通过 Claude Code 等 AI 工具辅助开发——AI 时代，学生参与校园信息化的门槛已经大幅降低。}
...
\midrule
Canvas LMS & API（需用户获取 Token）\\
教学信息服务网、phycai、MOOC、AI 好课 & 模拟登录 + 爬虫 \\
campuslife API & \textbf{公开接口（无需认证）}\\
教务处、水源社区、交大邮箱 & 网页抓取 / IMAP readonly \\
\bottomrule
\end{tabularx}
...
\textbf{帮同学省时间，不做越界的事。}不做考勤监控、不分析行为、不推送广告。\textbf{数据主权在学生。}随时导出、随时删除、可以选择不参与任何数据收集。
